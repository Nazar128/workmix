...
export default async function TasksPage({
  ...
  return (
    <div className="p-4 md:p-10 min-h-screen bg-[#fafaff] relative overflow-hidden">
      ...
      <header className="max-w-6xl mx-auto flex flex-col justify-between items-start md:items-center gap-6 mb-12">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight mb-2 ">
            <span className="text-purple-600">GÖREV</span>LERİM
          </h1>
          <p className="text-slate-500 text-sm md:font-medium">Bugün odaklanman gereken işler burada.</p>
        </div>
      ...
      <div className=" md:max-w-6xl mx-auto bg-white/70 backdrop-blur-xl rounded-[2rem] border border-white shadow-2xl shadow-slate-200/50 overflow-hidden">
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-50/50 border-b border-slate-100">
              <th className="p-2 md:px-8 md:py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Durum</th>
              <th className="p-2 md:px-8 md:py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Görev Detayı</th>
              <th className="p-2 md:px-8 md:py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Proje / Öncelik</th>
              <th className="p-2 md:px-8 md:py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-right">İşlemler</th>
            </tr>
          </thead>
            ...
            {!tasksRes.data || tasksRes.data.length === 0 ? (
              ...
              tasksRes.data.map((task: any) => (
                <tr key={task.id} className="group hover:bg-purple-50/40 transition-all duration-300">
                  <td className="p-2 md:px-8 md:py-6">
                  ...
                  <td className="p-2 md:px-8 md:py-6">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-2 text-[9px] md:text-[11px] font-black text-slate-500 uppercase tracking-tight">
                        <Layers className="w-3.5 h-3.5" />
                        {task.projects?.name || "Bireysel"}
                      </div>
                      <div className={`inline-flex items-center gap-1.5 p-1 md:px-3 rounded-full border text-[8px] md:text-[10px] font-black uppercase tracking-widest w-fit ${getPriorityColor(task.priority)}`}>
