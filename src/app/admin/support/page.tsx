...
import { User, Clock, CheckCircle2, MessageSquare, Inbox, Search, Send, Shield, Sparkles, HelpCircle } from "lucide-react";
...
export default function AdminSupportPanel() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-tr from-slate-100 via-slate-50 to-indigo-100/40 dark:from-[#030303] dark:via-[#0a0a12] dark:to-[#120d26] font-sans antialiased text-slate-900 dark:text-zinc-50 flex flex-col h-screen overflow-hidden p-0 sm:p-5">
      ...
      <div className="flex flex-col h-full w-full max-w-[1600px] mx-auto bg-white/70 dark:bg-[#0d0d15]/50 backdrop-blur-2xl border border-slate-200/50 dark:border-white/[0.06] sm:rounded-2xl shadow-[0_20px_50px_rgba(15,23,42,0.04)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.6)] overflow-hidden">
        ...
        <header className="h-16 border-b border-slate-200/60 dark:border-white/[0.05] bg-white/40 dark:bg-[#0f0f1b]/40 backdrop-blur-md px-6 flex items-center justify-between shrink-0 z-10">
          <div className="flex items-center gap-3.5">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 flex items-center justify-center shadow-lg shadow-indigo-500/20 dark:shadow-purple-500/20">
        ...
        <div className="flex flex-1 overflow-hidden w-full">
          ...
          <aside className="w-full md:w-[380px] lg:w-[420px] border-r border-slate-200/60 dark:border-white/[0.05] bg-white/20 dark:bg-[#0b0b14]/20 flex flex-col shrink-0 overflow-hidden h-full">
            <div className="p-4 border-b border-slate-100 dark:border-white/[0.03] space-y-3">
          ...
          <main className="flex-1 bg-slate-50/10 dark:bg-[#07070c]/10 flex flex-col overflow-hidden h-full">
            {selectedTicket ? (
              <div className="flex flex-col h-full overflow-hidden">
                ...
                <div className="h-14 border-b border-slate-200/60 dark:border-white/[0.05] bg-white/40 dark:bg-[#0d0d15]/40 px-6 flex items-center justify-between shrink-0">
                  <div className="min-w-0">
                    <h2 className="text-xs font-bold text-slate-800 dark:text-zinc-100 truncate tracking-wide">{selectedTicket.subject}</h2>
                    <div className="flex items-center gap-2 text-[10px] text-slate-400 dark:text-zinc-500 mt-0.5 font-medium">
                      <span className="font-mono bg-slate-200/50 dark:bg-white/[0.04] px-1.5 py-0.5 rounded text-slate-600 dark:text-zinc-300">Kullanıcı: {selectedTicket.user_id}</span>
                      <span>•</span>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gradient-to-b from-transparent to-slate-100/20 dark:to-black/20">
                  
                  <div className="flex gap-3 max-w-2xl">
                    <div className="w-8 h-8 rounded-xl bg-slate-200 dark:bg-white/[0.04] border border-slate-300/30 dark:border-white/[0.02] flex items-center justify-center shrink-0 text-xs font-bold shadow-sm text-slate-600 dark:text-zinc-300">
                      U
                    </div>
                    <div className="space-y-1">
                      <div className="bg-white dark:bg-[#11111e]/90 px-4 py-3 rounded-2xl rounded-tl-none border border-slate-200/60 dark:border-white/[0.04] shadow-[0_4px_20px_rgba(0,0,0,0.01)] dark:shadow-[0_4px_25px_rgba(0,0,0,0.2)]">
                  ...
                  {replyFromAdmin && (
                    <div className="flex gap-3 max-w-2xl ml-auto flex-row-reverse">
                      <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 text-white flex items-center justify-center shrink-0 text-xs font-bold shadow-md shadow-indigo-500/10">
                        A
                      </div>
                      <div className="space-y-1 text-right">
                        <div className="bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-700 dark:from-indigo-500 dark:via-indigo-600 dark:to-purple-600 text-white px-4 py-3 rounded-2xl rounded-tr-none shadow-lg shadow-indigo-500/5 dark:shadow-indigo-500/10 text-left border border-indigo-400/20">
                ...
                <div className="p-4 bg-white/40 dark:bg-[#0d0d15]/40 border-t border-slate-200/60 dark:border-white/[0.05] shrink-0 backdrop-blur-md">
                  {!isSelectedClosed ? (
                    <div className="relative bg-white/90 dark:bg-[#07070c]/90 border border-slate-200 dark:border-white/[0.06] rounded-xl focus-within:border-indigo-500 dark:focus-within:border-indigo-400 focus-within:ring-4 focus-within:ring-indigo-500/5 transition p-2 flex flex-col gap-2 shadow-sm">
                      ...
                      <div className="flex items-center justify-end border-t border-slate-100 dark:border-white/[0.03] pt-2 px-1">
                        <button
                          ...
                          className="bg-gradient-to-r from-indigo-600 via-indigo-500 to-purple-600 hover:opacity-95 text-white font-semibold text-xs px-4 py-2 rounded-xl shadow-md shadow-indigo-500/10 transition-all disabled:opacity-30 flex items-center gap-1.5 active:scale-95"
                        >
                    ...
                    <div className="flex items-center justify-center gap-2 text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 dark:bg-emerald-500/5 py-3 rounded-xl border border-emerald-500/20 text-xs font-semibold backdrop-blur-md shadow-sm">
                      <CheckCircle2 className="w-4 h-4" /> Bu destek talebi başarıyla çözüldü ve arşivlendi.
                    </div>
                  )}
                </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-[11px] text-slate-400 dark:text-zinc-500 max-w-[260px] mt-1 font-medium leading-relaxed">Detayları ve konuşma akışını canlı görüntülemek için sol menüden bir bilet seçin.</p>
              </div>
            )}
          </main>
