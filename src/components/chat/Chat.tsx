...
export default function ChatTest() {
  ...
  if (!userOrgId) return (
    <div className="w-full text-white bg-red-900/20 p-4 rounded-xl border border-red-500/50">
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden justify-between">
      <div className="flex-1 overflow-y-auto space-y-2 mb-4 pr-1 text-white">
            ...
            <div key={msg.id}>
              {showDay && (
                <div className="flex items-center gap-2 my-2">
                  ...
                  <span className="text-[10px] text-purple-200">{dayLabel}</span>
                  <div className="flex-1 h-px bg-white/10" />
                </div>
              )}
              <div className={`flex gap-2 group items-end ${isOwn ? "flex-row-reverse" : ""}`}>
                ...
                <div className={`flex flex-col max-w-[80%] ${isOwn ? "items-end" : "items-start"}`}>
                  {!isOwn && <span className="text-[10px] text-purple-200 mb-1 ml-1">{name}</span>}
                  {editingId === msg.id ? (
                    ...
                    <div className={`px-3 py-2 rounded-2xl text-sm ${isOwn ? "bg-purple-600/40 text-white rounded-tr-none" : "bg-white/10 text-gray-200 rounded-tl-none"} ${msg.pinned ? "ring-1 ring-yellow-500/40" : ""}`}>
                      ...
                      {msg.updated_at && <span className={`text-[10px] ${isOwn ? "text-blue-200" : "text-gray-400 ml-1"}`}>(düzenlendi)</span>}
                    </div>
                  )}
                  <div className={`flex items-center gap-1 mt-0.5 ${isOwn ? "flex-row-reverse" : ""}`}>
                    <span className="text-[10px] text-purple-300/60">{formatTime(msg.created_at)}</span>
                    <div className="hidden group-hover:flex items-center gap-1">
                      {isOwn && editingId !== msg.id && (
                        <button onClick={() => { setEditingId(msg.id); setEditText(msg.content ?? ""); }} className="text-gray-400 hover:text-blue-400 text-xs">✏️</button>
                      )}
                      {isOwn && (
                        <button onClick={() => handleDelete(msg.id)} className="text-gray-400 hover:text-red-400 text-xs">🗑️</button>
                      )}
                      {canPin && (
                        <button onClick={() => handlePinToggle(msg.id, !!msg.pinned)} className={`text-xs ${msg.pinned ? "text-yellow-400" : "text-gray-400 hover:text-yellow-400"}`}>📌</button>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {previewImage && (
        <div className="flex items-center gap-2 mb-2 px-1 bg-white/5 p-2 rounded-lg">
          <Image src={previewImage} alt="önizleme" width={40} height={40} className="w-10 h-10 rounded-lg object-cover" />
          <span className="text-xs text-purple-200 flex-1">Görsel yükendi</span>
          <button onClick={() => setPreviewImage(null)} className="text-gray-400 hover:text-white">✕</button>
        </div>
      )}
      <div className="flex gap-2 items-center pt-2 border-t border-white/5">
        <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="text-purple-300 hover:text-white transition">
        ...
        <input
          ...
          className="flex-1 bg-white/10 border border-white/10 rounded-xl px-3 py-2 text-white placeholder-purple-200/40 outline-none focus:border-white/20 text-sm"
          placeholder="Mesaj yaz..."
        />
        <button 
          onClick={handleSend} 
          disabled={(!text.trim() && !previewImage) || uploading} 
