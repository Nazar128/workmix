...
export default function AddTaskModal({ projects }: { projects: any[] }) {
  ...
  if (!isOpen) return <button onClick={() => setIsOpen(true)} className="bg-blue-600 text-white px-4 py-2 rounded">Yeni Görev Ekle</button>;

  return (
    <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white p-6 rounded-lg w-full max-w-md border border-gray-300 shadow-xl">
        <h2 className="text-xl text-purple-800 font-bold mb-4 border-b pb-2">Yeni Görev Oluştur</h2>
        <form action={action} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            <button type="button" onClick={() => setIsOpen(false)} className="px-4 py-2 bg-gray-100 rounded border text-red-700">İptal</button>
